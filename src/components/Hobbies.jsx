...
// Cai do céu com spring — igual ao AboutMe
  ...
  return (
    <button onClick={onClick}
      className="fixed top-4 right-4 z-[10000] w-10 h-10 rounded-full
        bg-slate-800/95 border-2 border-slate-600 flex items-center justify-center
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        <motion.div className="flex flex-col items-center gap-3 max-w-5xl w-full"
          initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          exit={{ scale: 0.92, opacity: 0 }}
          transition={{ ease: [0.16,1,0.3,1], duration: 0.32 }}
          onClick={(e) => e.stopPropagation()}>
          <img src={src} alt={caption} className="max-w-full max-h-[85vh] w-auto rounded-2xl shadow-2xl object-contain" />
          {caption && <p className="font-mono text-xs text-slate-400 tracking-widest text-center">{caption}</p>}
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
...
function VideoThumb({ src, className }) {
  ...
  useEffect(() => {
    ...
    video.playsInline = true; video.currentTime = 0.5;
  ...
  if (!thumb) return (
    <div className={`${className} bg-slate-900 flex items-center justify-center`}>
      <svg className="w-4 h-4 text-slate-600" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
    </div>
  );
...
function MediaItem({ item, color }) {
  ...
  if (item.type === "image") return (
    <>
      <motion.div className="relative rounded-xl overflow-hidden cursor-zoom-in group/img bg-slate-950"
        whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }} onClick={() => setLightbox(true)}>
        <img src={item.src} alt={item.caption} className="w-full object-contain max-h-[500px]" />
        <div className="absolute inset-0 bg-black/0 group-hover/img:bg-black/20 transition-all duration-300 flex items-center justify-center">
          <div className="opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center">
  ...
  return (
    <>
      <div className="relative rounded-xl overflow-hidden bg-slate-950 group/vid cursor-pointer"
        ...
        {item.src ? <VideoThumb src={item.src} className="w-full object-contain max-h-[500px]" />
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover/vid:bg-black/30 transition-colors duration-200">
          <motion.div className={`w-14 h-14 rounded-full ${c.bg} border-2 ${c.border} backdrop-blur-sm flex items-center justify-center`}
            whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
            <svg className={`w-6 h-6 ${c.text} ml-0.5`} viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
          </motion.div>
        </div>
      ...
      {videoModal && item.src && <VideoModal src={item.src} caption={item.caption} onClose={() => setVideoModal(false)} />}
    </>
  );
}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {media.map((m, i) => (
            <button key={i} onClick={() => setActive(i)}
              className={`relative flex-shrink-0 w-16 h-12 rounded-lg overflow-hidden border-2 transition-all duration-200 bg-slate-950
...
// ── Accordion com spring na entrada
function HobbyAccordion({ hobby, index, isOpen, onToggle }) {
  ...
  const glowBackground = useTransform([glowX,glowY], ([x,y]) =>
    `radial-gradient(circle at ${x} ${y}, ${c.glow} 0%, transparent 65%)`
  );
  ...
  return (
    <motion.div
      ref={cardRef}
      // ── Cai do céu com spring, cada card com delay escalonado
      variants={dropIn(index * 0.1, SPRING_BOUNCE)}
      initial="hidden"
      whileInView="visible"
      exit="exit"
      ...
      className="group relative rounded-2xl border border-slate-700/60 bg-slate-900/50
        overflow-hidden transition-colors duration-300 hover:border-slate-600/80"
    >
      <motion.div className="pointer-events-none absolute inset-0 rounded-2xl z-0
      ...
      <button onClick={onToggle}
        ...
        <motion.div animate={isOpen ? { scale: [1,1.15,1] } : { scale: 1 }}
          transition={isOpen ? { duration: 0.4 } : {}}
          className={`flex-shrink-0 w-10 h-10 rounded-xl ${c.bg} border ${c.border} flex items-center justify-center`}>
          {hobby.icon}
        </motion.div>
        <div className="flex-1 min-w-0">
          <p className={`font-mono text-[0.58rem] ${c.text} tracking-widest uppercase mb-0.5`}>{hobby.label}</p>
          <h4 className="font-syne font-bold text-slate-100 text-sm sm:text-base leading-snug">{hobby.title}</h4>
        </div>
        <span className={`hidden sm:flex flex-shrink-0 items-center gap-1 font-mono text-[0.6rem] ${c.text} ${c.bg} border ${c.border} px-2 py-0.5 rounded-full`}>
          <svg className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 3v18" />
          </svg>
          {hobby.media.length}
        </span>
        <div className="flex-shrink-0 w-7 h-7 rounded-full border border-slate-600 flex items-center justify-center text-slate-400
          group-hover:border-slate-500 group-hover:text-slate-300 transition-all duration-200">
          <motion.svg animate={{ rotate: isOpen ? 45 : 0 }} transition={{ duration: 0.3, ease: [0.16,1,0.3,1] }}
      ...
      <div className="px-4 sm:px-5 relative z-10">
        <div className={`h-px ${c.bar} rounded-full transition-opacity duration-500 ${isOpen ? "opacity-100" : "opacity-30"}`} />
      </div>

      <div className="relative z-10 transition-[grid-template-rows] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
        style={{ display: "grid", gridTemplateRows: isOpen ? "1fr" : "0fr" }}>
        <div className="overflow-hidden">
          <div className="transition-[opacity,transform] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
            style={{ opacity: isOpen ? 1 : 0, transform: isOpen ? "translateY(0)" : "translateY(-8px)" }}>
            <div className="px-4 sm:px-5 py-4 flex flex-col gap-5">
              <p className="text-slate-400 text-sm font-mono leading-relaxed">{hobby.description}</p>
              <MediaGallery media={hobby.media} color={hobby.color} isOpen={isOpen} />
              <div className="flex flex-wrap gap-2">
                {hobby.tags.map((tag) => (
                  <span key={tag} className={`font-mono text-[0.62rem] ${c.text} ${c.bg} border ${c.border} px-2.5 py-1 rounded-full`}>
                    {tag}
                  </span>
                ))}
              </div>
  ...
  return (
    <section id="hobbies" className="relative px-6 sm:px-8 pt-16 sm:pt-20 pb-12 overflow-hidden bg-slate-800">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        {/* ── Título — cai do céu primeiro (delay 0) */}
        <motion.div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_200px] gap-8 lg:gap-12 items-start">
          ...
          {/* ── Cards — cada um com delay escalonado após o título */}
          <div className="flex flex-col gap-3">
