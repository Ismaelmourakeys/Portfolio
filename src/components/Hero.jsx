// Hero.jsx com parallax no vídeo + animações de entrada via Framer Motion
...
export default function Hero() {
  ...
  // ── useScroll: rastreia o scroll dentro da seção
  // scrollYProgress vai de 0 (topo da seção) a 1 (saiu da tela)
  ...
  // ── useTransform: mapeia o scrollYProgress para um valor de translateY
  // Quando scrollYProgress = 0 → y = 0px (posição normal)
  // Quando scrollYProgress = 1 → y = 80px (vídeo se moveu menos que a página)
  // Isso cria o efeito de parallax — o vídeo "flutua" mais devagar
  const videoY = useTransform(scrollYProgress, [0, 1], ["0px", "80px"]);

  // ── Variantes para stagger do bloco de texto
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-12 sm:gap-16">
        ...
        {/* VÍDEO com parallax
            motion.div envolve o container do vídeo
            style={{ y: videoY }} aplica o parallax calculado pelo useTransform */}
        <motion.div
          className="relative flex-shrink-0"
          initial={{ opacity: 0, x: -40 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          style={{ y: videoY }} // ← parallax: vídeo sobe mais devagar que a página
        >
          {/* anéis decorativos */}
        ...
        {/* TEXTO com stagger
            motion.div com variants + whileInView anima os filhos em sequência */}
        <motion.div
