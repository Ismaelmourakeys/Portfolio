...
// ─────────────────────────────────────────────────────────────
// ── DURAÇÃO TOTAL DA TELA ────────────────────────────────────
// Altere EXIT_DELAY para mudar quando a tela começa a sair:
//   2500 = Rápido  (~2-3s)
//   4800 = Médio   (~4-5s)  ← atual
//   7000 = Dramático (~6-8s)
...
// ─────────────────────────────────────────────────────────────
// ── LINHAS DE TEXTO ──────────────────────────────────────────
// Edite aqui para mudar o conteúdo da tela de boas-vindas.
// delay    = quando a linha começa a aparecer (ms desde o início)
// type     = duração da digitação (ms)
// glitch   = duração do glitch antes de digitar (ms)
// color    = classe Tailwind de cor do texto
// size     = classe Tailwind de tamanho
// mono     = true = fonte monospace, false = fonte display
const LINES = [
  {
...
// ─────────────────────────────────────────────────────────────
// Hook: glitch → decifra → digita para uma string
// ─────────────────────────────────────────────────────────────
const GLITCH_CHARS = "█▓▒░⣿⠿⡟╋┼╬▪◈◆⬡✦✧⋆∴∵";
  ...
  useEffect(() => {
    ...
    timeout = setTimeout(() => {
      ...
      const glitchLoop = (now) => {
        ...
        // Letras da esquerda vão se fixando progressivamente
        ...
        if (progress < 1) {
          ...
          // Fase de digitação letra por letra
          setPhase("type");
          let i = 0;
...
// ─────────────────────────────────────────────────────────────
// Canvas: fundo espacial com estrelas + nebulosa + cometas leves
// ─────────────────────────────────────────────────────────────
function SpaceBg() {
  ...
  useEffect(() => {
    ...
    // Nebulae — pré-renderizadas no offscreen (não mudam)
    ...
    nebulae.forEach((n) => {
      const g = offCtx.createRadialGradient(
        n.x * offscreen.width, n.y * offscreen.height, 0,
        n.x * offscreen.width, n.y * offscreen.height, n.r
      );
      g.addColorStop(0, `rgba(${n.color.join(",")},${n.alpha})`);
      g.addColorStop(1, `rgba(${n.color.join(",")},0)`);
      offCtx.beginPath();
      offCtx.arc(n.x * offscreen.width, n.y * offscreen.height, n.r, 0, Math.PI * 2);
    ...
    const stars = Array.from({ length: 140 }, () => {
      ...
      return {
        ...
        color: Math.random() > 0.88
          ...
          ? "rgba(167,139,250,"
          : "rgba(200,210,240,",
        minAlpha: 0.05 + Math.random() * 0.1,
        maxAlpha: 0.3  + depth * 0.45,
      };
    });
    ...
    class Comet {
      ...
      reset(initial = false) {
        ...
        this.type  = roll < 0.55 ? 0 : roll < 0.85 ? 1 : 2;
        const rng  = (a, b) => a + Math.random() * (b - a);
        const sizes = [
          { len: [25,55],   speed: [1.2,2.5], width: [0.3,0.55], op: [0.1,0.22] },
          { len: [80,150],  speed: [2.5,4.0], width: [0.6,1.1],  op: [0.25,0.42] },
          { len: [170,260], speed: [3.5,5.5], width: [1.1,1.9],  op: [0.42,0.68] },
        ];
        ...
        this.delay     = initial ? Math.floor(Math.random() * 400) : 300 + Math.floor(Math.random() * 1000);
      }
      update() {
      ...
      draw() {
        ...
        g.addColorStop(0.55, `rgba(${this.hue},${(this.opacity*0.3).toFixed(3)})`);
        g.addColorStop(1,    `rgba(${this.hue},${this.opacity.toFixed(3)})`);
        ...
        if (this.type > 0) {
          const gr = this.width * (this.type === 2 ? 5.5 : 3.5);
          const gw = ctx.createRadialGradient(this.x, this.y, 0, this.x, this.y, gr);
          gw.addColorStop(0, `rgba(${this.hue},${(this.opacity*0.9).toFixed(3)})`);
          gw.addColorStop(1, `rgba(${this.hue},0)`);
    ...
    const draw = (t) => {
      ...
      fc++; if (fc % 2 !== 0) return; // 30fps para poupar recurso

      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(offscreen, 0, 0);

      stars.forEach((s) => {
        const a = s.minAlpha + (s.maxAlpha - s.minAlpha) * (0.5 + 0.5 * Math.sin(t * s.speed * 60 + s.phase));
        ctx.beginPath();
...
// ─────────────────────────────────────────────────────────────
// Linha individual de texto com efeito completo
// ─────────────────────────────────────────────────────────────
function TextLine({ text, delay, typeDuration, glitchDuration, color, size, mono, onDone }) {
  const { display, phase } = useDecipherText(text, {
    startDelay: delay,
    glitchDuration,
    typeDuration,
    onDone,
  ...
  return (
    ...
    >
      {display}
      {/* cursor piscante durante glitch e digitação */}
...
export default function WelcomeScreen({ onDone }) {
  ...
  // ── Som de boas-vindas ───────────────────────────────────────
  // Pré-carrega o arquivo e toca logo quando a tela abre.
  // Troque o caminho abaixo pelo seu arquivo .wav / .mp3
  const welcomeAudioRef = useRef(null);
  useEffect(() => {
    const audio = new Audio("/assets/audio/welcome_sound.wav"); // ← altere o caminho aqui
    audio.preload = "auto";
    audio.volume  = 0.5;        // ← ajuste o volume (0.0 a 1.0)
    audio.load();
    welcomeAudioRef.current = audio;

    // Toca imediatamente — junto com o primeiro glitch
    const t = setTimeout(() => {
      try {
        audio.currentTime = 0;
        audio.play();
      } catch (e) {
        console.warn("Erro ao reproduzir som de boas-vindas:", e);
      }
    }, 50); // 50ms de margem para garantir que o componente já montou

    return () => clearTimeout(t);
  }, []);
  // ────────────────────────────────────────────────────────────

  // Agenda a saída após EXIT_DELAY
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  return (
    <AnimatePresence onExitComplete={onDone}>
      {visible && (
        ...
        >
          {/* ── Fundo espacial (estrelas + nebulosa + cometas) */}
          <SpaceBg />
          ...
          <div
            ...
            style={{
              ...
              background: "radial-gradient(circle, rgba(56,189,248,0.055) 0%, rgba(139,92,246,0.035) 40%, transparent 70%)",
              filter: "blur(45px)",
              top: "50%", left: "50%",
          ...
          <motion.div
            className="pointer-events-none absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-sky-400/25 to-transparent"
            animate={{ top: ["8%", "92%", "8%"] }}
          ...
          {[
            ...
            <motion.div
              key={i}
              className={`absolute w-7 h-7 border-sky-400/35 ${cls}`}
          ...
          {/* ── Indicador de coordenadas (canto inferior esquerdo) */}
          <motion.div
            className="absolute bottom-5 left-8 font-mono text-[10px] text-slate-700 tracking-widest"
          ...
          <motion.div
            className="absolute top-5 right-10 flex items-center gap-1.5 font-mono text-[10px] text-slate-600 tracking-widest"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          >
            <div
              className="border border-sky-400/18 rounded-xl bg-slate-950/85 backdrop-blur-sm overflow-hidden"
