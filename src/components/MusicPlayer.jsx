...
const MUSIC_TITLE = "Ismael Moura"; // nome exibido
const MUSIC_SUBTITLE = "composição pessoal"; // subtítulo
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < BAR_COUNT; i++) {
        ...
        // cor varia por posição — azul → violeta → verde
...
export default function MusicPlayer({ visible = true, autoUnlocked = false }) {
  ...
  // toca quando a página carrega E o áudio foi desbloqueado pelo clique no loader
  useEffect(() => {
          ...
          >
            ...
            <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-20">
              {[...Array(6)].map((_, i) => (
                <motion.div
                  key={i}
                  className="absolute w-0.5 h-0.5 rounded-full bg-sky-400"
            ...
            <AnimatePresence mode="wait">
              {minimized ? (
                /* ── versão minimizada — só botão play + ondinha */
                <motion.div
                  key="mini"
                ...
                >
                  <button
                    ...
                    className="w-8 h-8 rounded-full flex items-center justify-center
                      ...
                      text-secondary hover:bg-secondary/30 transition-colors duration-200 cursor-pointer"
                  >
                    {playing ? (
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                >
                  {/* header: info + controles de janela */}
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2.5">
                      {/* ícone de nota musical animado */}
                      <motion.div
                        className="w-8 h-8 rounded-lg bg-secondary/10 border border-secondary/20
                          flex items-center justify-center flex-shrink-0"
                        animate={playing ? {
                          boxShadow: ["0 0 0px rgba(56,189,248,0)", "0 0 12px rgba(56,189,248,0.3)", "0 0 0px rgba(56,189,248,0)"],
                        } : {}}
                  ...
                  {/* barra de progresso */}
                  <div
                    className="relative h-1 bg-slate-800 rounded-full cursor-pointer mb-2 overflow-hidden"
                    onClick={handleSeek}
                  >
                    <motion.div
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <motion.button
                      ...
                      className="w-10 h-10 rounded-full flex items-center justify-center
                        ...
                        text-secondary hover:bg-secondary/25
                        transition-colors duration-200 cursor-pointer flex-shrink-0"
                      whileHover={{ scale: 1.08 }}
                      whileTap={{ scale: 0.95 }}
                      animate={playing ? {
                        boxShadow: ["0 0 0px rgba(56,189,248,0)", "0 0 16px rgba(56,189,248,0.25)", "0 0 0px rgba(56,189,248,0)"],
                      } : {}}
                      transition={{ duration: 2, repeat: Infinity }}
                    >
                      {playing ? (
                    ...
                    {playing && (
                      ...
                      >
                        {[...Array(3)].map((_, i) => (
                          <motion.div
                            key={i}
                            className="w-0.5 rounded-full bg-secondary/60"
